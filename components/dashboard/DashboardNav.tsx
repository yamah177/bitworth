...
import { LogOut, RefreshCw, FolderDown } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { CurrencySelector } from '@/components/ui/CurrencySelector'
import { AuthDataModal } from '@/components/ui/DataModal'
...
export function DashboardNav({ userEmail, btcPrice, currency, onCurrencyChange, onRefresh, refreshing, onImported }: Props) {
      ...
      <nav className="sticky top-0 z-40 bg-[#0a0a0a]/90 backdrop-blur-sm border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          ...
          <div className={`hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-mono transition-colors duration-300 ${
            priceFlash ? 'bg-[#F7931A]/20 border-[#F7931A]/40 text-[#F7931A]' : 'bg-white/5 border-white/10 text-white/50'
          }`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleLogout} disabled={loggingOut} title="Log out"
              className="w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-white hover:bg-white/5 transition-colors">
              <LogOut size={14} />
            </button>
          </div>
