...
import { GuestAssetsList } from '@/components/dashboard/GuestAssetsList'
import { GuestLiabilitiesList } from '@/components/dashboard/GuestLiabilitiesList'
...
export function GuestDashboardClient() {
  ...
  const fetchData = useCallback(async () => {
    const [accts, liabs, snaps, curr] = await Promise.all([
      guestApi.getAccounts(),
      guestApi.getLiabilities(),
      guestApi.getSnapshots(365),
      guestApi.getCurrency(),
    ])
    setAccounts(accts as Account[])
    setLiabilities(liabs as Liability[])
    setSnapshots(snaps as NetWorthSnapshot[])
    setCurrency(curr as CurrencyCode)
  }, [])
  ...
  const handleCurrencyChange = useCallback(async (code: CurrencyCode) => {
    setCurrency(code)
    await guestApi.setCurrency(code)
  }, [])
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-4 animate-fade-in">
        ...
        {isEmpty && (
          <div className="bg-[#F7931A]/5 border border-[#F7931A]/15 rounded-2xl p-8 text-center">
            ...
            <p className="text-white/40 text-sm max-w-sm mx-auto">
              You&apos;re in guest mode — your data stays on this device. No account needed.
              Add your first asset to start tracking your net worth in Bitcoin.
            </p>
          </div>
        )}
        <NetWorthHero
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2">
            <GuestAssetsList accounts={accounts} btcPrice={usdPrice} currency={currency} onRefresh={handleRefresh} />
          </div>
          <div className="space-y-4">
            <AllocationBreakdown accounts={accounts} btcPrice={usdPrice} />
          </div>
        </div>
        <GuestLiabilitiesList liabilities={liabilities} btcPrice={usdPrice} currency={currency} onRefresh={handleRefresh} />
        {!isEmpty && (
          <>
            <BtcInsightCard netWorthUsd={netWorthUsd} netWorthBtc={netWorthBtc} btcPrice={usdPrice} />
            <BtcPercentileCard key={netWorthBtc.toFixed(4)} netWorthBtc={netWorthBtc} />
          </>
        )}
        <div className="pb-6" />
      </main>
